...
// Sluit het instellingenmenu zodra er ergens anders in de app wordt geklikt,
// of bij een druk op Escape. Prettiger dan alleen een kruisje.
const menuContainer = ref(null)

function sluitMenuBijKlikBuiten(gebeurtenis) {
  if (toonMenu.value && menuContainer.value && !menuContainer.value.contains(gebeurtenis.target)) {
    toonMenu.value = false
  }
}

function sluitMenuBijEscape(gebeurtenis) {
  if (gebeurtenis.key === 'Escape') toonMenu.value = false
}

onMounted(() => {
  document.addEventListener('click', sluitMenuBijKlikBuiten)
  document.addEventListener('keydown', sluitMenuBijEscape)
})

onUnmounted(() => {
  document.removeEventListener('click', sluitMenuBijKlikBuiten)
  document.removeEventListener('keydown', sluitMenuBijEscape)
})
</script>
...
<!-- Logo van de opdrachtgever. Nu een plaatshouder; wordt later
          ...
          <div class="header-acties">
              ...
              <div class="menu-container-header relative" ref="menuContainer"></div>
